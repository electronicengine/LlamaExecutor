...
void LlamaExecutor::setOptions(const std::string & modelPath, int ngl, int n_ctx, float minP, float temp, int topK, float topP, bool embedding) {
...
void LlamaExecutor::initializeModel() {
    ...
    ctx_params.embeddings = _embedding;
...
void LlamaExecutor::batch_add_seq(llama_batch & batch, const std::vector<int32_t> & tokens, llama_seq_id seq_id)
{
...
void LlamaExecutor::batch_decode(llama_context * ctx, llama_batch & batch, float * output, int n_seq, int n_embd, int embd_norm) {
...
std::vector<float> LlamaExecutor::calculateEmbeddings(const std::string& text) {
